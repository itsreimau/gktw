...
class MessageCollector extends Collector {
    ...
    async _collect(msg) {
        ...
        if (!msg.key.fromMe && (this.jid === msg.key.remoteJid || this.hears.includes(msg.key.remoteJid)) && content?.length) {
            ...
            const sender = await Functions.getSender(msg, this.clientReq.self.core);
            return {
                content,
                ...msg,
                ...
                media: {
                    toBuffer: async () => await this.clientReq.getMediaMessage({
                        message
                    }, "buffer"),
                    toStream: async () => await this.clientReq.getMediaMessage({
                        message
                    }, "stream")
                }
            };
