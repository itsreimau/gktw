...
class Client {
    constructor(opts) {
        ...
        this.duplicateMessage = new NodeCache({
            stdTTL: 60,
            useClones: false
        });
    ...
    onMessage() {
        try {
            ...
            for (const [jid, pushName] of Object.entries(this.pushNames)) {
                if (jid.endsWith("@lid")) {
                    delete this.pushNames[jid];
                }
            }

            for (const [lidJid, realJid] of Object.entries(this.jids)) {
                if (lidJid.endsWith("@lid")) {
                    delete this.jids[lidJid];
                }
            }
        } catch (error) {
            fs.writeFileSync(this.jidsPath, JSON.stringify(this.jids));
        ...
        this.core.ev.on("messages.upsert", async (m) => {
            ...
            if (this.duplicateMessage.get(message.key.id)) return;
            this.duplicateMessage.set(message.key.id, true);
            ...
            let senderJid = await Functions.getSender(msg, this.core);

            if (msg.key?.remoteJid.endsWith("@g.us") && senderJid.endsWith("@lid")) {
                if (this.jids[senderJid]?.endsWith("@s.whatsapp.net")) {
                    msg.key.participant = this.jids[senderJid];
                    senderJid = this.jids[senderJid];
                } else {
                    const metadata = await this.core.groupMetadata(msg.key.remoteJid);
                    const participant = metadata.participants.find(p => this.decodeJid(p.jid) === senderJid);
                    if (participant) {
                        this.jids[senderJid] = participant.jid;
                        this.saveJids();
                        msg.key.participant = participant.jid;
            ...
            const used = ExtractEventsContent(msg, msgType);
            const ctx = new Ctx({
                used,
                args: [],
                self,
            ...
            if (MessageEventList[msgType]) await MessageEventList[msgType](msg, this.ev, self, this.core);
