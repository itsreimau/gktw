        ...
        this.core.ev.on("messages.upsert", async (m) => {
                ...
                if (msg.key?.remoteJid.endsWith("@g.us") && senderJid.endsWith("@lid")) {
                    if (this.jids[senderJid]?.endsWith("@s.whatsapp.net")) {
                        ...
                        const participant = metadata.participants.find(p => this.decodeJid(p.jid) === senderJid; this.jids[senderJid] = participant.jid; this.saveJids();

                            msg.key.participant = participant.jid; senderJid = participant.jid;
                        }
                    }
